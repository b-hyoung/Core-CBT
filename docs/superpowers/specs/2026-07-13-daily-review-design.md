...
3. **저장 전 자동 검증 (이 레포 담당)**:
   - 필수 필드 존재 (question_text, answer 비어있지 않음)
   - 정답이 문제 본문에 그대로 노출되지 않는지 문자열 검사
   - 원본과 문제 텍스트가 사실상 동일하면(유사도 과다) 거부 — "같은 문제 반복 금지" 위반
4. **풀이 화면에 "문제 이상해요" 버튼**: 원탭으로 `status='discarded'` 처리 + 다음 생성 때
   해당 원본의 새 변형 생성. 사람 최종 판정을 가장 싸게 넣는 장치.
5. 검증 실패 변형은 저장하지 않고 생성 결과 요약에 "N건 품질 미달로 제외" 표시.
