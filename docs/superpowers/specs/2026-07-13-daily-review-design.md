...
### 변형의 폭 — 복습함 구성 정책 (사용자 확정)

약점의 단위는 두 층이다: **문항**(틀린 바로 그 문제)과 **개념/서브토픽**(권한관리,
집계, JOIN 등). "GRANT 문제를 틀리면 시스템이 '권한관리 약함'을 기억했다가 이후
권한 관련 문제를 계속 내줘야 한다."

- **문항 변형 (주력, ~80%)**: 틀린 문제의 구조 변형 — 측정 개념은 유지하되
  표면(테이블·값)과 구조(COUNT→SUM/AVG, 조건 변형)를 함께 다양화
- **개념 확장 (약점 겨냥)**: 오답의 개념 태그를 집계해 약한 개념을 파악하고,
  **같은 개념의 다른 기출**을 뽑아 그것의 변형을 출제
  (예: GRANT 틀림 → 이후 복습함에 권한관리 계열 문제가 계속 등장)
- **커버리지 (~20%)**: 무작위가 아니라 **미시도 우선** — "한 번도 안 푼 개념 →
  시도 횟수 적은 개념 → 무작위" 순서로 선정. 오답 기록이 없는 개념은 강점이
  아니라 **미지의 약점**으로 취급한다 (콜드 스타트와 사각지대를 동시에 해결).

구현 방식: 확장·커버리지 문제도 자유 생성이 아니라 **기출을 앵커로 그것의 변형을
생성** — 모든 생성물에 품질 게이트가 동일하게 적용된다.
비율 규칙: 오답 N건 → 문항 변형 N건 + 개념 확장/커버리지 `ceil(N × 0.25)`건
(개념 확장 우선, 남으면 미시도 커버리지).

주의(사용자 지적): 약점 겨냥 루프만으로는 위험하다 — 학습 전(콜드 스타트)에는
데이터가 없고, 학습 후에도 안 풀어본 유형이 사각지대로 남는다. 대응:
(a) 위의 미시도 우선 커버리지, (b) **기출 회차 풀이가 전 범위 커버리지의 본체**이고
오늘의 복습은 보조 수단임을 전제로 한다. 복습함이 유일한 학습 경로가 아니다.
(선택 UI: 복습 페이지에 "아직 안 풀어본 개념 N개" 카운터 노출 — 시간 남으면.)

### 개념 태그 — 사전 배치 태깅 (신규 필요)

"같은 개념의 다른 기출"을 찾으려면 기출 전체에 개념 태그가 필요하다.
현 데이터셋의 `subcategory`는 이론(네트워크 등)은 세분화돼 있으나 SQL은 뭉뚱그려져
있음(`query`).

- 1회성 스크립트로 실기 기출 전체에 개념 태그 부여 (LLM 배치 라벨링)
  → `datasets/practicalIndustrial/conceptTags.json` 으로 커밋 (사람 검수 가능)
- 태그 체계(초안): SQL — DCL/권한, DML/집계, JOIN, 서브쿼리, DDL, 트랜잭션 등 /
  Code — 언어별 제어흐름, 배열/포인터, OOP 등 / 이론 — 기존 subcategory 재사용
- 오답 기록 + 태그 조인 → 개념별 오답 카운트 = 약점 프로필
