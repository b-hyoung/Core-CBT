...
시험의 SQL/Code 슬롯은 매회 다른 서브토픽이 나온다. 순수 표면 치환만으로는 부족하다.

- **주력 (~80%): 틀린 개념의 구조 변형** — 측정 개념(예: 집계 후 필터링)은 유지하되
  표면(테이블·값)뿐 아니라 구조도 다양화 (COUNT→SUM/AVG, HAVING 조건 변형 등)
- **인접 주제 확장**: 오답이 있는 대분류마다 같은 카테고리의 **다른 서브토픽** 문제 1개
  (예: GROUP BY 틀림 → GRANT 권한관리 확장 문제)
- **커버리지 (~20%)**: 같은 카테고리(SQL이면 SQL) 전체에서 무작위 서브토픽 출제

구현 방식: 확장·커버리지 문제도 자유 생성이 아니라 **같은 카테고리의 다른 기출을
무작위로 뽑아 그것의 변형을 생성** — 모든 생성물이 기출에 앵커되어 품질 게이트가
동일하게 적용된다. 비율 규칙: 오답 N건 → 변형 N건 + 확장/커버리지 `ceil(N × 0.25)`건.
