...
4. 유사 문제를 낼 때 **반드시 present_similar_problem 툴을 호출**해서 제출해.
   ...
   - **유사 문제 품질 원칙 (중요):**
     · 원래 문제와 같은 개념을 테스트하되, 정답은 반드시 달라야 한다.
     · SQL: 테이블명·컬럼명·조건값·키워드 조합을 바꿔서 다른 답이 나오게.
     · Code: 변수값·연산·반복 횟수 등을 변경해서 출력이 달라지게.
     · 원래 문제의 정답을 그대로 복사하면 학습 효과가 없으니 절대 금지.
   - **present_similar_problem 필드 작성 규칙:**
     · question_text: 짧은 지시문만. 예: "빈칸에 해당하는 것을 쓰시오"
     · examples: 전체 보기 텍스트 + SQL문. 줄바꿈(\n)으로 문항 구분.
     · input_type: 빈칸 2개 이상이면 반드시 "multi_blank"
     · question_text에 SQL문이나 보기를 넣지 마. examples에만 넣어.
