...
export default function ThemeControls() {
  ...
  const moveTheme = (direction) => {
    ...
    setActiveTheme((currentTheme) => {
      const currentIndex = themeOrder.findIndex((themeId) => themeId === currentTheme);
  ...
  return (
    <div className="flex items-center gap-2">
      <div
        className={`overflow-hidden rounded-full transition-[max-width,opacity] duration-300 [transition-timing-function:cubic-bezier(0.25,1,0.5,1)] motion-reduce:transition-none ${
      ...
      >
        <div
          ...
          className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-white/90 px-2 py-1 shadow-sm outline-none dark:border-slate-700 dark:bg-slate-900/90"
