...
export default async function PracticalHighUnknownPage({ searchParams }) {
  ...
  if (statsRows) {
    const statsMin2 = rankProblemUnknownRatesFromStats(statsRows, { minAttempts: 2 });
    usingMinAttempts = statsMin2.length > 0 ? 2 : 1;
    stats = statsMin2.length > 0 ? statsMin2 : rankProblemUnknownRatesFromStats(statsRows, { minAttempts: 1 });
  } else {
    const problemOutcomes = await readProblemOutcomes();
    const fromOutcomesMin2 = aggregateProblemUnknownRatesFromOutcomes(problemOutcomes, { minAttempts: 2 });
  ...
  if (stats.length === 0) {
    ...
    const legacyMin2 = aggregateProblemUnknownRates(events, { minAttempts: 2 });
    usingMinAttempts = legacyMin2.length > 0 ? 2 : 1;
    stats = legacyMin2.length > 0 ? legacyMin2 : aggregateProblemUnknownRates(events, { minAttempts: 1 });
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          <h1 className="mb-3 text-2xl font-extrabold text-slate-900">실기 모르겠어요 많은 문제 풀기</h1>
          <p className="mb-6 text-slate-600">아직 실기 문항별 모르겠어요 집계가 충분하지 않습니다.</p>
  ...
  return (
    <PracticalQuizV2
      ...
      session={{
        title: `실기 모르겠어요 많은 문제 풀기 (${picked.length}문제)`,
        reviewOnly: true,
        lobbySubtitle: `모르겠어요 비율 상위 문항 / 최소 ${usingMinAttempts}회 시도`,
      }}
      sessionId="practical-high-unknown"
