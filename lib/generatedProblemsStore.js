...
// 풀이 결과 반영: 맞힘 → done(졸업·아카이브 보존) / 틀림·모름 → 내일 재출제
// 아카이브(done)를 다시 풀다 틀리면 pending으로 재소환된다.
...
export async function applyDailyReviewOutcomes(userEmail, outcomes) {
  ...
  for (const o of outcomes) {
    ...
    const correct = Boolean(o?.isCorrect);
    const patch = correct
      ? { status: 'done', last_result_at: new Date().toISOString() }
      : { status: 'pending', due_date: kstTomorrowString(), last_result_at: new Date().toISOString() };
    // 맞힘은 pending → done만, 틀림은 done도 pending으로 재소환 (discarded는 불변)
    const statusFilter = correct ? 'status=eq.pending' : 'status=neq.discarded';
    const url =
      `${baseUrl()}?user_email=eq.${encodeURIComponent(userEmail)}` +
      `&source_session_id=eq.${encodeURIComponent(sid)}` +
      `&source_problem_number=eq.${num}&${statusFilter}`;
