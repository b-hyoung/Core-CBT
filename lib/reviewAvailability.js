...
export async function getUtilityAvailability(examType) {
  ...
  if (statsRows) {
    const wrongMin2 = rankProblemWrongRatesFromStats(statsRows, { minAttempts: 2 });
    wrongRows = wrongMin2.length > 0 ? wrongMin2 : rankProblemWrongRatesFromStats(statsRows, { minAttempts: 1 });

    const unknownMin2 = rankProblemUnknownRatesFromStats(statsRows, { minAttempts: 2 });
    unknownRows =
      unknownMin2.length > 0 ? unknownMin2 : rankProblemUnknownRatesFromStats(statsRows, { minAttempts: 1 });
  } else {
