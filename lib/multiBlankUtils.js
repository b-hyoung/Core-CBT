...
export function getMultiBlankMeta(problem, correctAnswer = '') {
  ...
  // 2차: 문장 중간의 ①②③ 등 원문자 라벨 (GRANT ① ON ② TO ③ 같은 패턴)
  if (labels.length < 2) {
    const inlineMatches = [...source.matchAll(/[①-⑳]/g)];
    for (const m of inlineMatches) {
      const label = normalizeLabelToken(m[0]);
      if (!seen.has(label)) { seen.add(label); labels.push(label); }
    }
  }

  // 3차: (1) (2) (3) 같은 괄호 숫자 라벨 (문장 중간)
  if (labels.length < 2) {
    const parenMatches = [...source.matchAll(/\(\s*(\d+)\s*\)/g)];
  ...
  if (labels.length === 0) return null;  // 라벨을 못 찾으면 null → single 입력으로 처리
