...
// 허용 답안 정리 — 코드/SQL 출력 문제에는 "동의어"가 없다.
// 정답과 정규화(공백·대소문자 무시) 동치인 표기 변형만 남긴다. 용어(이론) 문제는 동의어 허용.
export function sanitizeAcceptedAnswers({ answer, acceptedAnswers, category }) {
  const base = String(answer);
  const list = [base, ...(Array.isArray(acceptedAnswers) ? acceptedAnswers.map(String) : [])]
    .filter(Boolean)
    .filter((v, i, arr) => arr.indexOf(v) === i);
  if (String(category) !== 'Code' && String(category) !== 'SQL') return list;
  const na = normalize(base);
  return list.filter((a) => normalize(a) === na);
}
...
export function buildGeneratorPrompt({ original, answer, comment, concept, failureReasons = [] }) {
  return [
    ...
    '- accepted_answers는 정답과 의미가 완전히 같은 표기만 허용 — 용어 문제는 동의어/영문 표기 가능하지만, 코드·SQL 출력 문제는 값이 다른 변형을 절대 넣지 말 것',
    '',
